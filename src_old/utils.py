...
def bin_to_ply_v2(nodes, labels, file_out):
    labels = np.array(labels, dtype=int)
    r, g, b = rgb_from_label(labels)
    len_nodes = len(nodes)
    header = \
        "ply\n" \
        "format binary_little_endian 1.0\n" \
...
if __name__ == "__main__":
    ...
    data_raw = "data"
    sequence = 4
    pts1 = "/media/cedric/Data/Documents/Datasets/kitti_velodyne/data/velodyne/dataset/sequences/00/velodyne/000000.bin"
    pts2 = "/home/cedric/Documents/Workspace/ML/PointNet/cxx/preprocess/cmake-build-debug/test.bin"
    lbls = "/media/cedric/Data/Documents/Datasets/kitti_velodyne/data/labels/dataset/sequences/00/labels/000000.label"

    points1 = np.fromfile(pts1, dtype=np.float32).reshape([-1, 4])
    points2 = np.fromfile(pts2, dtype=np.float32).reshape([-1, 4])

    print(points1[0, :])
    print(points2[0, :])

    bin_to_ply_v2(points2[:, :3], points2[:, 3], "test.ply")
